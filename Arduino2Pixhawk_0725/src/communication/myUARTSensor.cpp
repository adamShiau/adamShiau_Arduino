  ...
  while (serial_.available() > 0 && data_status == NO_DATA) {
    ...
    switch (state)
    {
      ...
      case EXPECTING_CHECKSUM:{
        ...
        const int MAX_MSG_LEN = 256;  // 固定長度緩衝區
        uint8_t all_message[MAX_MSG_LEN];
        
        if (length_msg > MAX_MSG_LEN) {
          Serial.println("ERROR: Message too long in getMeasures");
          data_status = NO_DATA;
          state = EXPECTING_HEADER;
          break;
        }
...
void XsensUart::PrintMessage(HardwareSerial &output_port){
  ...
  if (length > MAX_MSG_LEN) {
    output_port.println("ERROR: Message too long");
    return;
  }
...
void XsensUart::reqPortConfig(){
  ...
  int length = getMessageLength();
  if (length > MAX_MSG_LEN) {
    Serial.println("ERROR: Message too long in reqPortConfig");
    return;
  }
...
/***  Use below new readNMEA fn, add timeout
...
int readNMEA(HardwareSerial &serial_nmea, char *receivedChars, int buffer_size) {
  ...
  while (millis() - t0 < timeout) {
    while (serial_nmea.available()) {
      char incomingChar = serial_nmea.read();
  ...
  // timeout case
  Serial.println("NMEA read timeout");
  receivedChars[0] = '\0';
  return 0;
}
...
bool checkLOCOSYS_ACK(HardwareSerial &serial_nmea, const char* expected_header, unsigned long timeout_ms = 1000) {
  char receivedChars[256];
  unsigned long t0 = millis();

  while (millis() - t0 < timeout_ms) {
    int len = readNMEA(serial_nmea, receivedChars, sizeof(receivedChars));
    if (len > 0 && strstr(receivedChars, expected_header) != NULL) {
      Serial.print("Matched Header [");
      Serial.print(expected_header);
      Serial.print("]: ");
      Serial.println(receivedChars);
      return true;
    }
  ...
  Serial.print("Timeout waiting for header: ");
  Serial.println(expected_header);
  return false;
}

void printMatchingNMEA(HardwareSerial &serial_nmea, const char* expected_header) {
  char buffer[256];

  if (readNMEA(serial_nmea, buffer, sizeof(buffer)) > 0) {
    // 如果該行是以 expected_header 開頭
    if (strncmp(buffer, expected_header, strlen(expected_header)) == 0) {
      Serial.print("Matched NMEA: ");
                               ...
                               unsigned long ack_timeout_ms = 1000)
{
  for (int i = 0; i < max_retries; i++) {
    ...
    if (checkLOCOSYS_ACK(serial_nmea, expected_header, ack_timeout_ms)) {
      Serial.println("GNSS command acknowledged.");
      return true;
    }
  }

  Serial.println("GNSS command failed after retries.");
  return false;
}
...
//   while (millis() - last_data_time < quiet_time_ms) {
//     while (port.available()) {
//       port.read();
//       last_data_time = millis();  // 有資料就更新最後資料時間
//     }
//     delay(10);  // 等待資料進來或靜止結束
//   }
// }
