...
/*** for XLM550 test */
#define SFA_X 2.24042E-05 // include 2.048/2^23
#define SFB_X -80.75709061

#define SFA_Y 2.23992E-05
#define SFB_Y -80.74025289

#define SFA_Z 1.34702E-07
#define SFB_Z -2.624E-03
...
void setup() {
  ...
  float std_imu[] = {0.001, 0.001, 0.001, 0.0007, 0.0007, 0.0007};
  my_ekf.setInit(std_imu);
...
// set interrupt mode to None
  ...
  Serial.println("End of XLM550 init");
...
void output_mode_setting(byte &mux_flag, byte mode, byte &select_fn)
{
	if(mux_flag == MUX_OUTPUT)
	{
      ...
      case MODE_AFI_ATT: {
        output_fn = acq_afi_att;
        select_fn = SEL_AFI_ATT;
        rst_fn_flag = MODE_AFI_ATT;
        break;
      }
      default: break;
    }
  ...
  // my_float_t tt;
	...
	if(run_fog_flag) {
      ...
      if(ISR_PEDGE)
      {
        ...
        XLM550_readData_f(my_ADXL357.float_val, acc_temp.float_val); //get xlm550 accl data in g
...
void acq_afi_att(byte &select_fn, unsigned int value, byte ch)
{
  byte *fog_x, *fog_y, *fog_z;
  static my_acc_t my_ADXL357, ADXL357_cali;
  static my_acc_t my_FOG_cali_MSB, my_GYRO_cali, my_att;
	uint8_t CRC32[4];
  my_acc_t acc_temp, acc_temp2;
  float att_dt_f;
  ...
  // my_float_t tt;
	...
	if(select_fn&SEL_AFI_ATT)
	{
    Serial.println("Enter acq_afi_att mode: ");
    CtrlReg = value;
	...
	if(run_fog_flag) {
      ...
      if(ISR_PEDGE)
      {
        ...
        // gyro_cali(reg_fog_x, reg_fog_y, reg_fog_z); // perform fog mis-alignment calibration
        gyro_cali_f(my_FOG_cali_MSB.bin_val, my_GYRO_cali.float_val, reg_fog_x, reg_fog_y, reg_fog_z); // perform fog mis-alignment calibration
        ...
        // memcpy(imu_data+ 8, reg_fog_y+8, 4); //fog_y
        // memcpy(imu_data+12, reg_fog_z+8, 4); //fog_z
        memcpy(imu_data+4, my_FOG_cali_MSB.bin_val, 12); //wx, wy, wz
        memcpy(imu_data+16, ADXL357_cali.bin_val, 12); //ax, ay, az
        // memcpy(imu_data+16, my_ADXL357.bin_val, 12); //ax, ay, az
        // memcpy(imu_data+28, acc_temp2.bin_val, 12); //XLM550 Tx, Ty, Tz
        memcpy(imu_data+28, reg_fog_x+12, 4); //Temp_x
        ...
        memcpy(imu_data+36, reg_fog_z+12, 4); //Temp_z
        memcpy(imu_data+40, mcu_time.bin_val, 4);
        memcpy(imu_data+44, my_att.bin_val, 12);
        myCRC.crc_32(imu_data, 56, CRC32);
        free(imu_data);
        ...
        if(data_cnt >= DELAY_CNT)
        {
          ...
          // Serial1.write(reg_fog_y+8, 4);
          // Serial1.write(reg_fog_z+8, 4);
          Serial1.write(my_FOG_cali_MSB.bin_val, 12);   //wx, wy, wz MSB
          Serial1.write(ADXL357_cali.bin_val, 12);      //ax, ay, az LSB
          // Serial1.write(acc_temp2.bin_val, 12); //XLM550 Tx, Ty, Tz
          Serial1.write(reg_fog_x+12, 4);
          Serial1.write(reg_fog_y+12, 4);
          Serial1.write(reg_fog_z+12, 4);
          ...
          Serial1.write(CRC32, 4);
          // Serial.println(tt.float_val);
          // Serial.print(my_att.float_val[0],4);
          // Serial.print(',');
          // Serial.print(my_att.float_val[1],4);
          // Serial.print(',');
          // Serial.println(my_att.float_val[2],4);
        }
        //  */
        #endif
        resetWDT();
        ...
        // Serial.print(',');
        // Serial.print(my_GYRO_cali.float_val[1]), 5;
        // Serial.print(',');
        // Serial.print(my_GYRO_cali.float_val[2], 5);
        // Serial.print(',');
        // Serial.print(ADXL357_cali.float_val[0],4);
        // Serial.print(',');
        // Serial.print(ADXL357_cali.float_val[1],4);
        // Serial.print(',');
        // Serial.println(ADXL357_cali.float_val[2],4);
...
void gyro_cali_f(byte fog_msb[12], float gyro_cli[3], byte gyro_clix[14], byte gyro_cliy[14], byte gyro_cliz[14])
{
  my_float_t x_f, y_f, z_f;
  my_float_t x_cli, y_cli, z_cli;
  ...
  fog_msb[0] = x_cli.bin_val[3];
  fog_msb[1] = x_cli.bin_val[2];
  fog_msb[2] = x_cli.bin_val[1];
  fog_msb[3] = x_cli.bin_val[0];
  fog_msb[4] = y_cli.bin_val[3];
  fog_msb[5] = y_cli.bin_val[2];
  fog_msb[6] = y_cli.bin_val[1];
  fog_msb[7] = y_cli.bin_val[0];
  fog_msb[8] = z_cli.bin_val[3];
  fog_msb[9] = z_cli.bin_val[2];
  fog_msb[10] = z_cli.bin_val[1];
  fog_msb[11] = z_cli.bin_val[0];
  
  gyro_cli[0] = x_cli.float_val;
  gyro_cli[1] = y_cli.float_val;
  gyro_cli[2] = z_cli.float_val;
...
void verify_output_fn(byte in)
{
  ...
  if(!( in==MODE_RST |in==MODE_FOG | in==MODE_IMU | in==MODE_FOG_HP_TEST |
    in==MODE_AFI | in==MODE_NMEA | in==MODE_FOG_PARAMETER | in==MODE_AFI_ATT ))
  {
...
void verify_select_fn(int in)
{
  ...
  Serial1.println(in);
  if(!( in==SEL_DEFAULT |in==SEL_RST | in==SEL_FOG_1 | in==SEL_FOG_2 | in==SEL_AFI | in==SEL_AFI_ATT |
