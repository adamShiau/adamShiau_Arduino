...
//=============================================================================================
// STOP 用：重置姿態（可選清偏置）
// 2026/01/28, change resetAttitude function, added (6) and (7)
void Madgwick::resetAttitude(bool resetBias) {
    // (1) reset quaternion to identity
    q0 = 1.0f; q1 = 0.0f; q2 = 0.0f; q3 = 0.0f;

    // (2) reset cached Euler (rad)
    roll = pitch = yaw = 0.0f;
    anglesComputed = 0;

    // (3) reset unwrap / angle history
    yaw_deg_prev = yaw_deg_cont = 0.0f;
    roll_deg_prev = 0.0f;
    has_prev_angles = false;

    // (4) reset dynamic beta LP state
    betaEffLP = beta;

    // (5) reset gyro bias (optional)
    if (resetBias) {
        bgx_dps = bgy_dps = bgz_dps = 0.0f;
    }

    // (6) reset trapezoidal gyro integration memory (important)
    hasPrevW = false;
    prevWx_rad = 0.0f;
    prevWy_rad = 0.0f;
    prevWz_rad = 0.0f;
