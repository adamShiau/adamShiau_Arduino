...
void SoftI2cMaster::init(uint8_t sdaPin, uint8_t sclPin) 
{
  ...
  timer = digitalPinToTimer(sdaPin_);
  if(timer != NOT_ON_TIMER) this->turnOffPWM(timer);
  timer = digitalPinToTimer(sclPin_);
  if(timer != NOT_ON_TIMER) this->turnOffPWM(timer);

  sdaport =digitalPinToPort(sdaPin_);
  sclport = digitalPinToPort(sclPin_);
  sdabit = digitalPinToBitMask(sdaPin_);
  sclbit = digitalPinToBitMask(sclPin_);
  ...
  cli();
  *sdareg |= sdabit;  // Set the SDA as OUTPUT
  *sdaout |= sdabit; // Set SDA HIGH
  *sclreg |= sclbit; // set the SCL as OUTPUT
  *sclout |= sclbit; // set the SCL HIGH
  SREG = oldSREG;
...
void SoftI2cMaster::turnOffPWM(uint8_t timer)
{
  switch(timer)
  {
