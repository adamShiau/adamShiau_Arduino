...
void processDataOutputs() {
  ...
  for (int i = 0; i < MAX_OUTPUTS; i++) {
    ...
    // ★ PKT_POSITION：單純靠 GGA 更新，不使用內部時鐘
    if (output->packet_type == PKT_POSITION) {
        ...
        // ✅ 封包建立和發送（不檢查 interval_ms）
    }
    // ★ PKT_HEADING：單純靠 HDT 更新，不使用內部時鐘
    else if (output->packet_type == PKT_HEADING) {
        uint32_t cur_epoch = gps->hdt_epoch;
        ...
        // 同一個HDT epoch 只送一次，避免重複
        if (cur_epoch == output->last_sent_hdt_epoch) {
            continue;
        }

        output->last_sent_hdt_epoch = cur_epoch;

        // ✅ 封包建立和發送（不檢查 interval_ms）
    }
    else {
        // ★ 其他封包類型：使用時間間隔控制
        if (current_time - output->last_send_time < output->interval_ms) {
            continue;
        }
    }
...
void command_init(void) {
  ...
  for (int i = 0; i < MAX_OUTPUTS; i++) {
    ...
    data_outputs[i].last_sent_gga_epoch = 0;
    data_outputs[i].last_sent_hdt_epoch = 0;
  }
}