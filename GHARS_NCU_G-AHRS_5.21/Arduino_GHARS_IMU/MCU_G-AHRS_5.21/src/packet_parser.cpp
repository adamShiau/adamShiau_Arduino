        ...
        switch (result->packet_type) {
            ...
            case PKT_POSITION:
                ...
                if (result->time_valid) {
                    ...
                    Serial.print(".");
                    if (result->millisecond < 100) Serial.print("0");
                    if (result->millisecond < 10) Serial.print("0");
                    Serial.print(result->millisecond);
                    Serial.print(" ");
...
// 解析 POSITION 封包 (v1.3 - 包含高精度UTC時間)
bool PacketParser::parsePositionPacket(uint8_t* data, uint16_t size, GnssData* result) {
    if (size != 29) return false;  // 新格式: 8+8+4+9 bytes

    result->latitude = readDouble(data, 0);    // 緯度 8 bytes
    result->longitude = readDouble(data, 8);   // 經度 8 bytes
    result->altitude = readFloat(data, 16);    // 海拔 4 bytes

    // 解析 UTC 時間 9 bytes (高精度)
    result->hour = data[20];                   // 小時
    result->minute = data[21];                 // 分鐘
    result->second = data[22];                 // 秒
    result->millisecond = readUint16(data, 23); // 毫秒 2 bytes
    result->day = data[25];                    // 日
    result->month = data[26];                  // 月
    result->year = readUint16(data, 27);       // 年 2 bytes
