...
// byte data_w[4]={10,11,12,13};
byte data_w[4], data_r[4];
...
void setup() {
  ...
  for(int i=0;i<4;i++) data_w[i] = i*i;
  eeprom.Info_Write(8, data_w, 4);
}
...
void loop() {
  ...
  eeprom.Read(8, &data1);
  eeprom.Read(9, &data2);
  eeprom.Read(10, &data3);
  eeprom.Read(11, &data4);
  eeprom.Parameter_Read(1,my_f.bin_val);
  eeprom.Info_Read(8,data_r, 4);
  ...
  Serial.print(data_r[0]);
  Serial.print(", ");
  Serial.print(data_r[1]);
  Serial.print(", ");
  Serial.print(data_r[2]);
  Serial.print(", ");
  Serial.print(data_r[3]);
  Serial.println("");
