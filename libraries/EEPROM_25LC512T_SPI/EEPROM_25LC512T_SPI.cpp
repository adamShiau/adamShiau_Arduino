#include <Arduino.h>
#include <Wire.h>
#include <EEPROM_25LC512_I2C.h>

using namespace std;

EEPROM_25LC512_I2C::EEPROM_25LC512_I2C(int eeprom_addres, TwoWire &p) : eeprom(eeprom_addres), myWire(p)
{

}

EEPROM_25LC512_I2C::EEPROM_25LC512_I2C(TwoWire &p) : myWire(p)
{

}

void EEPROM_25LC512_I2C::begin() {
    myWire.begin();
}

void EEPROM_25LC512_I2C::Parameter_Read(unsigned int eeaddress, unsigned char* buf) {
	unsigned char i=0;
	eeaddress <<= 2;
...
void EEPROM_25LC512_I2C::Read(unsigned int eeaddress, unsigned char* buf) {
	myWire.beginTransmission(eeprom);
	myWire.write(eeaddress >> 8);   // MSB
	myWire.write(eeaddress & 0xFF); // LSB
	myWire.endTransmission();
...
void EEPROM_25LC512_I2C::read(unsigned int eeaddress, unsigned char* data, unsigned int num_chars) {
  unsigned char i=0;
  myWire.beginTransmission(eeprom);
  myWire.write(eeaddress >> 8);   // MSB
  myWire.write(eeaddress & 0xFF); // LSB
 ...
 */
void EEPROM_25LC512_I2C::Parameter_Write(unsigned int eeaddress, int value) {
	eeaddress <<= 2;
	myWire.beginTransmission(eeprom);
	myWire.write(eeaddress >> 8);   // MSB
	myWire.write(eeaddress & 0xFF); // LSB
...
void EEPROM_25LC512_I2C::Write(unsigned int eeaddress, char value) {
	myWire.beginTransmission(eeprom);
	myWire.write(eeaddress >> 8);   // MSB
	myWire.write(eeaddress & 0xFF); // LSB
	myWire.write(value);
...
void EEPROM_25LC512_I2C::write(unsigned int eeaddress, char* data) {
