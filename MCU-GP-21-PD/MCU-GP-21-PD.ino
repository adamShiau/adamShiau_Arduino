...
void acq_fog2(byte &select_fn, unsigned int value, byte ch)
{
  ...
  String fpga_version;
	...
	if(run_fog_flag) {
      ...
      if(fog)
      {
        uint8_t* imu_data = (uint8_t*)malloc(15); // KVH_HEADER:4 + pig:14
        
        memcpy(imu_data, KVH_HEADER, 4);
        // memcpy(imu_data+4, fog, 14);
        memcpy(imu_data+4, fog, 11);
        myCRC.crc_32(imu_data, 15, CRC32);
        free(imu_data);

        #ifdef UART_RS422_CMD
        // char angle[6];
        // dtostrf(160.12, 6, 2, angle);
        Serial1.write(KVH_HEADER, 4);
        Serial1.write(fog, 11);
