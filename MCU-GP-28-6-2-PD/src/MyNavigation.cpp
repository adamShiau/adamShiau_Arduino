...
namespace Navigation{
    ...
    void ComplementaryFilter::setIMUError(Sensor_ID sensor_id, int fs){
        ...
        for (int i=0;i<3;i++){
            acc_error += br[i] * br[i];
            acc_error += std[i] * std[i];
        }
        acc_error = 1 / sqrt(acc_error);
    ...
    void ComplementaryFilter::setDataRate(int fs){
        ...
        float error_accl[] = {sqrt(BS_ACCL[1] * BS_ACCL[1] + STD_ACCL[1] * STD_ACCL[1]) / 9.79, sqrt(BS_ACCL[0] * BS_ACCL[0] + STD_ACCL[0] * STD_ACCL[0]) / 9.79};
    ...
    void ComplementaryFilter::setEnableLC_Output(bool enable_LC_output){
        output_LC_flag = enable_LC_output;
    }
    ...
    void ComplementaryFilter::getCaliRate(const float (&omg)[3], float (&new_omg)[3]){
