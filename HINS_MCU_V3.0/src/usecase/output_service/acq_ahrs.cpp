...
static void ahrs_reset_runtime_state(void)
{
    ...
    data_cnt = 0;
    try_cnt  = 0;
    ax_lp = ay_lp = az_lp = 0.0f;

    g_w_prev_dps[0] = g_w_prev_dps[1] = g_w_prev_dps[2] = 0.0f;
    g_dt_prev_s = 0.0f;
    g_have_prev = 0;
    g_last_ts_us = 0;
    ...
    g_just_still   = 0;
    g_still_ts_ms  = 0;
    g_bias_alpha_base = 0.0f;
    // (keep current bias alpha inside ahrs_attitude; stop handler resets attitude)

    // ---- circular moving-average filter buffers ----
...
static void ahrs_stage_attitude_update(void)
{
    ...
    // 3-1.5) accel low-pass (for correction)
    ...
    // 3-2) compute dt (seconds)
    uint32_t now_us = micros();
    float dt_curr;
    if (g_last_ts_us == 0) {
        dt_curr = Ts;
    } else {
        uint32_t du = now_us - g_last_ts_us;
    ...
    // 3-3) two-sample coning -> equivalent rate
    float w_eq_dps[3];
    if (g_have_prev) {
    ...
    // 3-4) attitude update (Madgwick wrapper)
    ...
    // 3-5) pitch/roll
    my_att.float_val[0] = ahrs_attitude.getLocalCasePitch();
    my_att.float_val[1] = ahrs_attitude.getLocalCaseRoll();

    // 3-6) gimbal-lock hysteresis + projected yaw integration
