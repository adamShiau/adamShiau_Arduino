#line 1 "C:\\Users\\user\\Documents\\Arduino\\MCU-GP-28-4-PD\\src\\MyNavigation.cpp"
        ...
        if (LC.isBiasAvailable(LC_omg)){
            if (pre_time >=0){
                ...
                if (checkALByAutocorrelation(pr_acc)){
                    ...
                    float new_pr[2];
                    new_pr[0] = ori_temp[0] - weight[0] * normalizeAngle_float(ori_temp[0] - pr_acc(0));
                    new_pr[1] = ori_temp[1] - weight[1] * normalizeAngle_float(ori_temp[1] - pr_acc(1));
