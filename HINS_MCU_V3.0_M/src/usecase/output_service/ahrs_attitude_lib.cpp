                           ...
                           my_att_t* att_out)
{
    ...
    // 7) pitch/roll
    att_out->float_val[0] = ahrs_attitude.getLocalCasePitch();
    att_out->float_val[1] = ahrs_attitude.getLocalCaseRoll();
    ...
    if (!ctx->yaw_gl_locked && fabsf(pitch_deg_now) >= GL_ENTER_DEG) {
        ctx->yaw_gl_locked = 1;
        ctx->yaw_hold_deg = ahrs_attitude.getLocalCaseYaw();
    }
    ...
    if (ctx->yaw_gl_locked) {
        ...
        att_out->float_val[2] = ahrs_attitude.getLocalCaseYaw();
