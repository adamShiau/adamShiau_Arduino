...
void acq_imu(byte &select_fn, unsigned int value, byte ch)
{
    ...
    // Send CMD 1 (10Hz, Channel 1)
    if (!cmd1_sent) {
        cmd1_sent = true;
        byte data1[] = {0xAB, 0xBA, 0x01, 0x00, 0x00, 0x00, 0x0A, 0x01, 0x55, 0x56};
        Serial2.write(data1, sizeof(data1));
        expecting_packet = true;
    }

    // Send CMD 2 (4Hz, Channel 2) after a small delay
    if (!cmd2_sent && data_cnt > 10) {
        cmd2_sent = true;
        byte data2[] = {0xAB, 0xBA, 0x02, 0x00, 0x00, 0x00, 0x04, 0x02, 0x55, 0x56};
        Serial2.write(data2, sizeof(data2));
        expecting_packet = true;
    }

    // Check for incoming packets from MINS
