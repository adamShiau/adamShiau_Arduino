...
// ======================================================================================
// 2026/03/05: Reduced the number of function calls to improve program performance
void Madgwick::getLocalCaseEuler(float& roll_deg, float& pitch_deg, float& yaw_deg)
{
    if (!anglesComputed) computeAngles();

    // 1) q_LC
    float lc0, lc1, lc2, lc3;
    currentLocalCaseQuat(
        q0,q1,q2,q3,
        qc0,qc1,qc2,qc3,
        qwl0,qwl1,qwl2,qwl3,
        lc0,lc1,lc2,lc3
    ...
    // 2) 先用 q_LC 算 roll/pitch/yaw（base）
    float r, p, y;
    eulerZYX_from_quat_deg(lc0, lc1, lc2, lc3, r, p, y);

    // 3) yaw zero：只修正 yaw（不動 roll/pitch）
    if (yawZeroValid) {
        // 方案 B：用 q_adj 只算 yaw（保持你原本的 quaternion yaw-zero 定義）
        float aj0, aj1, aj2, aj3;
        quatMul(qz0w, qz0x, qz0y, qz0z, lc0, lc1, lc2, lc3, aj0, aj1, aj2, aj3);

        float rr, pp, yy;
        eulerZYX_from_quat_deg(aj0, aj1, aj2, aj3, rr, pp, yy);
        y = yy;
    }
