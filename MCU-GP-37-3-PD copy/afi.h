...
#include <vector>
#include "src/LinearCorrection.h"
#include "src/DataOutput.h"
#include "src/ComplementaryFilter.h"
...
#define PRINT_SELECT_FN(x) \
  ...
  if (x == SEL_DEFAULT) { \
      ...
      Serial1.println("SELECT SEL_DEFAULT"); \
  } else if (x == SEL_RST) { \
      Serial.println("SELECT SEL_DEFAULT"); \
      Serial1.println("SELECT SEL_DEFAULT"); \
  } else if (x == SEL_FOG_1) { \
      Serial.println("SELECT SEL_FOG_1"); \
      Serial1.println("SELECT SEL_FOG_1"); \
  } else if (x == SEL_FOG_2) { \
      Serial.println("SELECT SEL_FOG_2"); \
      Serial1.println("SELECT SEL_FOG_2"); \
  } else if (x == SEL_FOG_3) { \
      Serial.println("SELECT SEL_FOG_3"); \
      Serial1.println("SELECT SEL_FOG_3"); \
  } else if (x == SEL_IMU) { \
      Serial.println("SELECT SEL_IMU"); \
      Serial1.println("SELECT SEL_IMU"); \
  } else if (x == SEL_NMEA) { \
      Serial.println("SELECT SEL_NMEA"); \
      Serial1.println("SELECT SEL_NMEA"); \
  } else if (x == SEL_FOG_PARA) { \
      Serial.println("SELECT SEL_FOG_PARA"); \
      Serial1.println("SELECT SEL_FOG_PARA"); \
  } else if (x == SEL_AFI) { \
      Serial.println("SELECT SEL_AFI"); \
      Serial1.println("SELECT SEL_AFI"); \
  } else if (x == SEL_HP_TEST) { \
      Serial.println("SELECT SEL_HP_TEST"); \
      Serial1.println("SELECT SEL_HP_TEST"); \
  }\
  else { \
      Serial.println("SELECT_FN out of range!"); \
      Serial1.println("SELECT_FN out of range!"); \
  }
...
#define PRINT_OUTPUT_REG(x) \
    ...
    switch(x) { \
        case INT_SYNC: \
            ...
            Serial1.println("INT_SYNC"); \
            break; \
        case EXT_SYNC: \
            Serial.println("EXT_SYNC"); \
            Serial1.println("EXT_SYNC"); \
            break; \
        case STOP_SYNC: \
            Serial.println("STOP_SYNC"); \
            Serial1.println("STOP_SYNC"); \
            break; \
        case NMEA_MODE: \
            Serial.println("NMEA_MODE"); \
            Serial1.println("NMEA_MODE"); \
            break; \
        case HP_TEST: \
            Serial.println("HP_TEST"); \
            Serial1.println("HP_TEST"); \
            break; \
        default: \
            Serial.println("Reg value out of range!"); \
            Serial1.println("Reg value out of range!"); \
            break; \
    }
...
#define PRINT_MUX_FLAG(x) \
  ...
  Serial1.println("\nprint mux_flag"); \
  if(x == 0) {\
    Serial.println("MUX_OUTPUT"); \
    Serial1.println("MUX_OUTPUT"); \
  }\
  else if(x == 1) {\
    Serial.println("MUX_PARAMETER"); \
    Serial1.println("MUX_PARAMETER"); \
  }\
  else if(x == 2) {\
    Serial.println("MUX_ESCAPE"); \
    Serial1.println("MUX_ESCAPE"); \
  }
