...
bool NmeaParser::parseLine(const char* nmea_line) {
    ...
    if (strncmp(msg_type, "GGA", 3) == 0) {
        ...
        return parseRMC(nmea_line);  // RMC 提供日期資訊
    }
...
bool NmeaParser::parseHDT(const char* line) {
    ...
    // 記住舊的 heading 值
    float prev_heading = gps_data.heading;
    ...
    // ⭐ 只有「heading 真的變了」才推進 epoch（考慮浮點數誤差）
    if (fabs(gps_data.heading - prev_heading) > 0.001) {
        gps_data.hdt_epoch++;
    }
