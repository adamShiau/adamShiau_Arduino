...
void Wait_FPGA_Wakeup(byte fog_ch)
{
  ...
  while(!flag){
    if((millis()-t0)>500){
      ...
      manual_Wait_FPGA_Wakeup_escape(flag, fog_ch);
      t0 = millis();
    }
  } 
  Serial.print("ch: ");
...
void manual_Wait_FPGA_Wakeup_escape(byte &flag, byte fog_ch)
{
  ...
  if(flag){
    ...
    for(int i=0; i<255; i++) Serial1.read();//clear serial buffer
  }
  else{
    digitalWrite(MCU_LED, HIGH);
    delay(100);
