    ...
    if (rx -> run == 1) {
        ...
        if (pkt) {
            ...
            if (update_raw_data(pkt, &sensor_raw) == 0) {
                ...
                // 3-1) 閥值篩選，目前人為設定 1 dph ~= 0.0003 dps
                my_att_t my_GYRO_att_calculate;
                
                if(abs(my_GYRO_cali.float_val[0]) > 0.0003) my_GYRO_att_calculate.float_val[0] = my_GYRO_cali.float_val[0];
                else my_GYRO_att_calculate.float_val[0] = 0.0f;
                if(abs(my_GYRO_cali.float_val[1]) > 0.0003) my_GYRO_att_calculate.float_val[1] = my_GYRO_cali.float_val[1];
                ...
                ahrs_attitude.updateIMU(my_GYRO_att_calculate.float_val[0], my_GYRO_att_calculate.float_val[1], 
                    my_GYRO_att_calculate.float_val[2], my_ACCL_cali.float_val[0], my_ACCL_cali.float_val[1], 
                    my_ACCL_cali.float_val[2]);
